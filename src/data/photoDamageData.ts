	...
	return readCsv<RawPhotoDamageCSV, PhotoDamageDataRow>(
		"フォト火力データ.csv",
		{
			transformHeader: (header: string) => {
				return RAW_PHOTO_DAMAGE_CSV_HEADERS.some((knownHeader) => knownHeader === header)
					? header
					: "";
			},
		},
...
export async function getPhotoDamageDataByPhotoIdAndState(
	...
	const photoDamageData = await getPhotoDamageData();
	return photoDamageData.filter(
		(data) => data.photoId === photoId && data.changeState === changeState,
	);
}
