		...
		async (data: RawFriendsCSV[]) => {
			...
			const filledData = data.map((row) => {
				const parsed = {
					...
					miraclePlus: row["けものミラクル＋"] || "",
					specialMoveName: row.とくいわざ技名 || "",
					specialMoveEffect: row.とくいわざ効果 || "",
					waitSkillName: row.たいきスキル技名 || "",
					waitSkillEffect: row.たいきスキル効果 || "",
					waitSkillActivationRate: String(row.たいきスキル発動率 ?? ""),
					waitSkillActivationCount: String(row.たいきスキル発動回数 ?? ""),
					traitName: row.とくせい技名 || "",
					traitEffect: row.とくせい効果 || "",
					kisekitraitName: row.キセキとくせい技名 || "",
					kisekitraitEffect: row.キセキとくせい効果 || "",
					nanairoSkillName: row.なないろとくせい技名 || "",
					nanairoSkillEffect: row.なないろとくせい効果 || "",
					introductionText: row.自己紹介テキスト || "",
					zukanText: row.ずかんテキスト || "",
