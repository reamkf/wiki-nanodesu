...
describe("getPhotoData", () => {
	...
	describe("てるてるのおまじないのデータ", () => {
		...
		describe("基本情報", () => {
			...
			it("アイコンURLがhttps://image01.seesaawiki.jp/k/h/kemono_friends3_5ch/b1uIfnXFkT.pngである", () => {
				expect(samplePhotoData?.iconUrl).toBe(
					"https://image01.seesaawiki.jp/k/h/kemono_friends3_5ch/b1uIfnXFkT.png",
				);
			});

			it("アイコンURL(変化後)がhttps://image01.seesaawiki.jp/k/h/kemono_friends3_5ch/ynAIAP4pF4.pngである", () => {
				expect(samplePhotoData?.iconUrlChanged).toBe(
					"https://image01.seesaawiki.jp/k/h/kemono_friends3_5ch/ynAIAP4pF4.png",
				);
			});
			...
			it("とくせい(変化後)が正しい", () => {
				expect(samplePhotoData?.traitChanged).toBe(
					"自身の与ダメージが10%増加し~~&color(red){くたくた、ひやひや、ズキンズキン、からげんき状態にならない}",
				);
			});
		});
	...
	describe("動物フォトデータ", () => {
		describe("通常フレンズ(ウミネコ)", () => {
			...
			it("とくせい(変化前)が正しい", () => {
				expect(umiNekoPhotoData?.trait).toBe(
					"地形がみずべの場合、~~毎ターン味方全体のMPが1増加する",
				);
			});

			it("とくせい(変化後)が正しい", () => {
				expect(umiNekoPhotoData?.traitChanged).toBe(
					"地形がみずべの場合、~~毎ターン味方全体のMPが{2}増加する",
				);
			});
		});
		...
		describe("HCフレンズ(【わたしを頼ってね！】アカギツネ)", () => {
			...
			beforeAll(() => {
				akagitsunePhotoData = photoData.find(
					(photo) => photo.name === "【わたしを頼ってね！】アカギツネ",
				);
			});
		...
		describe("【青い瞳の迷い猫】マヌルネコ(フォト)", () => {
			...
			beforeAll(() => {
				manulNekoPhotoData = photoData.find(
					(photo) => photo.name === "【青い瞳の迷い猫】マヌルネコ(フォト)",
				);
			});
