import {
	getPhotoDamageData,
	getPhotoDamageDataByPhotoId,
	getPhotoDamageDataByPhotoIdAndState,
} from "@/data/photoDamageData";
...
describe("getPhotoDamageData", () => {
	...
	describe("JHR・パン食い競争の部のデータ", () => {
		let samplePhotoDamageData:
			| Awaited<ReturnType<typeof getPhotoDamageData>>[number]
			| undefined;

		beforeAll(() => {
			samplePhotoDamageData = photoDamageData.find(
				(data) => data.photoId === "JHR・パン食い競争の部" && data.changeState === "変化前",
			);
		});
	...
	describe("あなたにあげるわのデータ", () => {
		let samplePhotoDamageDataPre:
			| Awaited<ReturnType<typeof getPhotoDamageData>>[number]
			| undefined;
		let samplePhotoDamageDataPost:
			| Awaited<ReturnType<typeof getPhotoDamageData>>[number]
			| undefined;

		beforeAll(() => {
			samplePhotoDamageDataPre = photoDamageData.find(
				(data) => data.photoId === "あなたにあげるわ" && data.changeState === "変化前",
			);
			samplePhotoDamageDataPost = photoDamageData.find(
				(data) => data.photoId === "あなたにあげるわ" && data.changeState === "変化後",
			);
		});
		...
		describe("かけっこインターバル", () => {
			...
			beforeAll(() => {
				const kakekoData = photoDamageData.filter(
					(data) =>
						data.photoId === "かけっこインターバル" && data.changeState === "変化前",
				);
...
describe("getPhotoDamageDataByPhotoIdAndState", () => {
	...
	it("複数条件があるフォトでも正しくフィルタリングされる", async () => {
		const kakekoData = await getPhotoDamageDataByPhotoIdAndState(
			"かけっこインターバル",
			"変化前",
		);
	...
	it("存在しないフォトIDと変化状態では空配列が返される", async () => {
		const nonExistentData = await getPhotoDamageDataByPhotoIdAndState(
			"存在しないフォト",
			"変化前",
		);
