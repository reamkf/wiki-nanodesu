...
export const PhotoNameLink = memo(function PhotoNameLink({ photo, isChanged }: PhotoNameLinkProps) {
	...
	return (
		<div className="flex flex-col">
			<SeesaaWikiLink href={pageUrl} className="block text-md">
				<div className="text-xs text-gray-600 space-x-1 flex items-center">
					<span>☆{photo.rarity}</span>
					{isChanged !== undefined &&
						(isChanged ? (
							<span className="bg-pink-100 text-pink-500 px-[4px] py-[2px] text-[0.6rem] rounded-sm">
								変化後
							</span>
						) : (
							<span className="bg-gray-100 text-gray-700 px-[4px] py-[2px] text-[0.6rem] rounded-sm">
								変化前
							</span>
						))}
				</div>
