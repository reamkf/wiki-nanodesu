...
async function readLicenses(): Promise<LicenseEntry[]> {
	...
	return Object.entries(parsed)
		...
		.filter(
			(entry) => !(entry.name === "wiki-nanodesu" || entry.name.startsWith("wiki-nanodesu@")),
		)
...
export default async function Page() {
	...
	return (
		<>
			...
			<p className="mb-4">
				ゲーム内、
				<a href="https://x.com/kemono_friends3" target="_blank" rel="noopener noreferrer">
					公式X
				</a>
				または
				<a href="https://kemono-friends-3.jp" target="_blank" rel="noopener noreferrer">
					公式サイト
				</a>
				に由来する画像・文章等の著作権は、「けものフレンズプロジェクト２Ｇ」及び「SEGA」「アピリッツ」又はその関連団体に帰属します。
				<br />
				フレンズの属性アイコンは{" "}
				<a
					href="https://github.com/nanase/kf-assets"
			...
			{licenses.length === 0 ? (
				<p className="text-sm">
					ライセンス情報が見つからないのです。先にスクリプトで生成するのです。
				</p>
			) : (
						...
						<li key={item.name} className="border rounded-md p-3 bg-white">
							...
							{item.licenseText ? (
								<details className="mt-2">
									<summary className="cursor-pointer text-xs text-blue-600 underline">
										ライセンス全文を表示
									</summary>
