...
function calculateFriendsStatusRaw(
	...
	const yaseiValue =
		yasei === 0 ? 0 : yasei === 4 ? (yasei4 ?? 0) : yasei === 5 ? (yasei5 ?? 0) : 0;
	...
	if (lv <= 90) {
		return Math.ceil(
			Math.floor(((lv90Value - lv1Value) / 89) * (lv - 1) + lv1Value + yaseiValue) *
				rankCorrection,
		);
	} else {
		return Math.ceil(
			Math.floor(((lv99Value - lv90Value) / 9) * (lv - 90) + lv90Value + yaseiValue) *
				rankCorrection,
		);
	}
}
...
export function calculateFriendsStatus(
	...
	const isYaseiAvailable: boolean =
		yasei === 4
			? !isStatusNull(base.yasei4)
			: yasei === 5
				? !isStatusNull(base.yasei5)
				: yasei === 0
					? true
					: false;
	...
	// 初期ステータス: 計算なし
		...
		if (rank === 6 && yasei === 4 && !isStatusNull(friendsDataRow.status.status90)) {
			...
			rank === 6 &&
			yasei === 5 &&
			!isStatusNull(friendsDataRow.status.status90Yasei5)
		) {
		...
		if (rank === 6 && yasei === 4 && !isStatusNull(friendsDataRow.status.status99)) {
			...
			rank === 6 &&
			yasei === 5 &&
			!isStatusNull(friendsDataRow.status.status99Yasei5)
		) {
	...
	// Lv 2-89, 91-98
		...
		if (lv >= 2 && lv <= 89 && !isStatusNull(base.lv1)) {
			return calculateFriendsStatusRawForEachParam(friendsDataRow, lv, rank, yasei);
		}

		if (lv >= 91 && lv <= 98 && !isStatusNull(base.lv99)) {
			return calculateFriendsStatusRawForEachParam(friendsDataRow, lv, rank, yasei);
		}
	}
...
export async function getFriendsStatusList(): Promise<FriendsStatusListItemWithDisplayValue[]> {
	...
	for (const friend of friendsData) {
		...
		for (const statusType of statusTypes) {
			...
			const kemosuteWithCostume = calculateStatusWithCostume(
				kemosute,
				costumeBonus.kemosute ?? 0,
			);
			const hpWithCostume = calculateStatusWithCostume(
				statusType.status.hp,
				costumeBonus.hp ?? 0,
			);
			const atkWithCostume = calculateStatusWithCostume(
				statusType.status.atk,
				costumeBonus.atk ?? 0,
			);
			const defWithCostume = calculateStatusWithCostume(
				statusType.status.def,
				costumeBonus.def ?? 0,
			);
			...
			result.push({
				...
				sortValues: {
					...
					avoid:
						statusType.yasei === 5
							? (friend.status.avoidYasei5 ?? -Infinity)
							: (friend.status.avoid ?? -Infinity),
				...
				displayValues: {
					...
					avoid: ((avoid: number | null) => (avoid && toPercent(avoid, 1)) || "???%")(
						statusType.yasei === 5 ? friend.status.avoidYasei5 : friend.status.avoid,
					),
				},
				originalIndex: index++,
			});
