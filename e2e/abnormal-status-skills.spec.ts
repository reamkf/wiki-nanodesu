...
test.describe("状態異常スキル一覧ページ", () => {
	...
	test("ページタイトルが表示される", async ({ page }) => {
		await expect(page.locator("h2", { hasText: "状態異常スキル一覧" })).toBeVisible();
	});
	...
	test("状態異常カテゴリが表示される", async ({ page }) => {
		...
		for (const status of statuses) {
			await expect(page.getByText(status, { exact: false }).first()).toBeAttached();
		}
	});
...
test.describe("状態異常スキル一覧: 目次とURLハッシュ", () => {
	test("URLハッシュがリーフIDのとき、折りたたみが開きテーブルが表示される", async ({ page }) => {
		await page.goto(`./abnormal-status-skills#${encodeURIComponent(ABNORMAL_TOC_LEAF_ID)}`);
		const table = page.locator("table").first();
		await expect(async () => {
			await expect(table).toBeVisible({ timeout: 2000 });
		}).toPass({ timeout: 15000 });
	...
	test("目次で子カテゴリを選ぶと折りたたみが開きテーブルが表示される", async ({ page }) => {
