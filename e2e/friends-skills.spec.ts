...
test.describe("スキル別フレンズ一覧ページ", () => {
	...
	test("ページタイトルが表示される", async ({ page }) => {
		await expect(page.locator("h2", { hasText: "スキル別フレンズ一覧" })).toBeVisible();
	});
	...
	test("カテゴリセクションが表示される", async ({ page }) => {
		...
		for (const category of categories) {
			await expect(page.getByText(category, { exact: true }).first()).toBeAttached();
		}
	});

	test("折りたたみセクションを展開するとテーブルが表示される", async ({ page }) => {
		...
		const toggleButton = page.locator("button.MuiButton-sizeSmall.text-gray-500").first();
	...
	test("展開されたテーブルに適切なヘッダーが表示される", async ({ page }) => {
	...
	test("説明テキストに状態異常スキルページへのリンクがある", async ({ page }) => {
...
test.describe("スキル別フレンズ一覧: 目次とURLハッシュ", () => {
	test("URLハッシュがリーフIDのとき、折りたたみが開きテーブルが表示される", async ({ page }) => {
		await page.goto(`./friends-skills#${encodeURIComponent(FRIENDS_TOC_LEAF_ANCHOR)}`);
		const table = page.locator("table").first();
		await expect(async () => {
			await expect(table).toBeVisible({ timeout: 2000 });
		}).toPass({ timeout: 15000 });
	...
	test("目次のリーフを選ぶと折りたたみが開きテーブルが表示される", async ({ page }) => {
		await page.goto("./friends-skills");
		...
		await dialogPanel.getByPlaceholder("目次を検索...").fill(FRIENDS_TOC_LEAF_ANCHOR);
		// MUI ListItemButton は aria-label が付かないため、表記テキストで辿る
		await dialogPanel.getByText(FRIENDS_TOC_LEAF_ANCHOR, { exact: true }).click();
		const table = page.locator("table").first();
		await expect(async () => {
			await expect(table).toBeVisible({ timeout: 2000 });
		}).toPass({ timeout: 15000 });
